...
func _physics_process(_delta):
    ...
    var vertical_acceleration = acceleration.dot(neck_node.basis.y)
    y_scale_velocity += vertical_acceleration * movement_sensitivity.y

    var longitudinal_acceleration = acceleration.dot(neck_node.basis.z)
    angular_velocity.x += longitudinal_acceleration * movement_sensitivity.z

    var transverse_acceleration = acceleration.dot(neck_node.basis.x)
